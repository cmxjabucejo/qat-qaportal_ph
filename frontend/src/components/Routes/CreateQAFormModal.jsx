...
const CreateQAFormModal = ({ isOpen, onClose, onSuccess }) => {
    ...
    useEffect(() => {
      ...
      const fetchExistingFormNames = async () => {
        try {
          ...
          const names = response.data.map(row => row.QA_FORM_NAME?.toLowerCase()?.trim());
          setExistingFormNames(names);
          // console.log("✅ Existing QA_FORM_NAMEs loaded:", names);
        } catch (error) {
    ...
    const isDuplicateFormName = existingFormNames.includes(formData.formName.toLowerCase().trim());
      ...
      try {
        ...
        data.forEach((item) => {
          ...
          if (lob) {
            ...
            if (task && !uniqueAccountsMap.get(accountKey).lobs[lob].includes(task)) {
        ...
        setAccountOptions(Object.values(sortedAccountObj).map((item) => item.ACCOUNT));
    ...
    const fetchLobList = async (account) => {
      try {
        ...
        setLobOptions(lobs.sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase())));
      } catch (error) {
    ...
    const fetchTaskList = async (account, lob) => {
      try {
        ...
        setTaskOptions(tasks.sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase())));
      } catch (error) {
    ...
    const handleChange = (e, componentIndex, attrIndex, field) => {
      ...
      if (field === 'sectionTitle') {
        ...
        if (field === 'points' && isZTP) {
          ...
          newComponents[componentIndex].attributes[attrIndex][field] = e.target.value;
        }
      }
    ...
    const addZTP = () => {
        ...
        components: [
            ...formData.components,
            { sectionTitle: '', attributes: [{ name: '', points: '' }], isZTP: true },
        ],
        });
    };
    ...
    const addAttribute = (componentIndex) => {
        ...
        const isZTP = newComponents[componentIndex].isZTP;
          newComponents[componentIndex].attributes.push({ name: '', points: isZTP ? '100' : '' });
        setFormData({ ...formData, components: newComponents });
    };
    ...
    const handleSubmit = async () => {
      ...
      try {
        // 1️⃣ Save to db_qa_forms_list
        await axios.post(`${SERVER_URL}/api/qa_forms_list`, {
          ...
          if (component.isZTP) {
            ...
            component.attributes.forEach((attr) => {
              if (questionIndex <= 80) {
                ...
                fullPayload[`${qKey}_PTS_DEDUCTIBLE`] =
                  parseFloat(attr.points || "100");
                questionIndex++;
              }
            });
          } else {
        ...
        // 3️⃣ Submit to db_qa_forms_table
        await axios.post(`${SERVER_URL}/api/qa_forms_table`, fullPayload);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      {isSubmitting && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="flex flex-col items-center justify-center bg-white px-6 py-6 rounded-lg shadow-lg">
            ...
            <p className="text-sm text-gray-600 font-medium">Saving, please wait...</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white w-full max-w-6xl h-[90vh] overflow-y-auto p-6 rounded shadow-xl">
        ...
        />
          {isDuplicateFormName && (
            ...
            <select
                ...
                >
                <option value="" disabled select>Select an Account</option>
                {accountOptions.map((account, index) => (
                  <option key={index} value={account}>{account.toUpperCase()}</option>
                ))}
            </select>
        ...
        {nonZTPComponents.map((component, componentIndex) => (
          <div key={componentIndex} className="border p-4 mb-4 rounded relative">
            ...
            <input
              ...
              onChange={(e) => handleChange(e, componentIndex, null, 'sectionTitle')}
            />
            ...
            {component.attributes.map((attr, attrIndex) => (
              <div key={attrIndex} className="grid grid-cols-12 gap-2 mb-2 items-center">
                ...
                <input
                  ...
                  value={attr.name}
                  onChange={(e) => handleChange(e, componentIndex, attrIndex, 'name')}
                />
                <input
                  type="number"
                  ...
                  placeholder="Points"
                  value={attr.points}
                  onChange={(e) => handleChange(e, componentIndex, attrIndex, 'points')}
                />
            ...
            {(() => {
              const lastAttr = component.attributes[component.attributes.length - 1];
              const isLastBlank = !lastAttr.name?.trim() || !lastAttr.points?.toString().trim();
              const isDisabled = component.attributes.length >= 15 || isLastBlank;

              return (
                <button
                  onClick={() => addAttribute(componentIndex)}
                  className={`px-4 py-1 rounded mt-2 text-white ${
                    isDisabled ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600'
                  }`}
                  disabled={isDisabled}
                  title={isLastBlank ? "Please fill in the last attribute before adding more." : ""}
                >
                  + Add Attributes
                </button>
              );
        ...
        {ztpComponent && (
          <div className="border p-4 mb-4 rounded relative">
            <div className="flex justify-between items-center">
              ...
              <button
                onClick={() => removeComponent(formData.components.findIndex((c) => c.isZTP))}
                className="text-red-600 text-lg"
              >
                🗑️
              </button>
            ...
            {ztpComponent.attributes.map((attr, attrIndex) => (
              <div key={attrIndex} className="grid grid-cols-10 gap-2 mb-2 items-center">
                <div className="col-span-1">{attrIndex + 1}</div>
                <input
                  type="text"
                  className="border p-2 col-span-8"
            ...
            {(() => {
              const lastZTPAttr = ztpComponent.attributes[ztpComponent.attributes.length - 1];
              const isZtpLastBlank = !lastZTPAttr.name?.trim(); // only check name for ZTP
              const isZtpDisabled = ztpComponent.attributes.length >= 21 || isZtpLastBlank;
              ...
              return (
                <button
                  ...
                  className={`px-4 py-1 rounded mt-2 text-white ${
                    isZtpDisabled ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-600'
                  }`}
        ...
        <div className="flex gap-4 mt-4">
          ...
          <button
            ...
            disabled={isDuplicateFormName || isSubmitting || !formData.formName.trim()}
          >
