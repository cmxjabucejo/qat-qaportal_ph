...
const QAForms = () => {
      ...
      if (qMatch) {
        ...
        if (typeof questionText === "string") {
          ...
          if (compPrefixMatch) {
            ...
            if (!grouped[compId]) grouped[compId] = { title: "Untitled Component", questions: {} };
            if (!grouped[compId].questions[qId]) grouped[compId].questions[qId] = {};
  
            grouped[compId].questions[qId]["QUESTION"] = questionText.replace(`${rawPrefix} - `, "");
          }
        }
      }
    ...
    try {
      ...
      rawData.forEach((row) => {
        ...
        Object.entries(row).forEach(([key, value]) => {
          if (
            /^(C\d{3}_COMP|Q\d{3}_(QUESTION|PTS_DEDUCTIBLE)|C011_ZTP|ZTP_\d{2}_(QUESTION|PTS_DEDUCTIBLE))$/.test(key)
          ) {
  ...
  const handleFormChange = (e) => {
    ...
    const formDetails = rows.find(row => row.QA_FORM_NAME === selectedFormName);
    if (formDetails) {
  ...
  const handleEmployeeChange = (e) => {
    ...
    const selectedEmp = allEmployees.find(emp => emp.employeeId === selectedValue);
    if (selectedEmp) {
  ...
  const isSaveDisabled = (() => {
    ...
    const hasMissingObservations = Object.entries(groupedData).some(([compKey, compData]) => {
      const { questions } = compData;
      return Object.keys(questions).some(qId => !observations[qId]);
    });
  
    // Check if observation comments are empty
    const isCommentsEmpty = !formData.observationComments || formData.observationComments.trim() === "";
  ...
  const handleSaveForm = async () => {
    ...
    Object.entries(grouped).forEach(([compKey, compData]) => {
      ...
      Object.entries(questions).forEach(([qId, qVal]) => {
        ...
        auditFields[`${qId}_PTS_DEDUCTIBLE`] =
          observations[qId] === "Met" ? qVal.PTS_DEDUCTIBLE :
          observations[qId] === "Not Met" ? 0 :
          null;
      });
    });
    ...
    const auditPayload = {
      ...
      SUPERVISOR_ID: isManualEntry
        ? manualSupervisorId
        : allEmployees.find(emp => emp.employeeId === selectedEmployeeId)?.supervisorId || '',
      SUPERVISOR_NAME: isManualEntry
        ? manualSupervisorName
        : allEmployees.find(emp => emp.employeeId === selectedEmployeeId)?.supervisorName || '',
  ...
  const isGenerateDisabled = isFormGenerated || !(
    selectedForm &&
    selectedEmployeeId &&
    formData.evaluationType &&
    (!isManualEntry || (
    ...
    <div className="h-screen overflow-hidden bg-[#f5f7fa] flex flex-col">
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-96 border-r border-gray-200 bg-white/80 p-4 space-y-4">
          ...
          <div className="space-y-4 justify-start">
            ...
            <div className="w-full">
              <label htmlFor="qa-form-dropdown" className="block text-sm font-bold text-gray-700 text-left">
                Select QA Form:
              </label>
              <select
                id="qa-form-dropdown"
            ...
            {/* Agent Name Dropdown */}
            <div className="w-full">
              <label htmlFor="employee-select" className="block text-sm font-bold text-gray-700 text-left">
                Agent Name:
              </label>
              <select
                id="employee-select"
            ...
            <div className="w-full">
              <label htmlFor="evaluation-type" className="block text-sm font-bold text-gray-700 text-left">
                Evaluation Type:
              </label>
              <select
                id="evaluation-type"
        ...
        <div className="bg-white p-6 rounded-lg shadow-md mb-4 w-full h-full sm:w-2/3 lg:w-2/3 lg:min-w-[80%] ml-4 overflow-y-auto overflow-x-hidden max-h-[100vh]">
          ...
          {isFormGenerated && selectedForm && (() => {
            const selectedFormData = rows.find(f => f.QA_FORM_NAME === selectedForm);
            if (!selectedFormData || !selectedFormData.components) return null;
            ...
            return (
              <div className="p-6 mb-4 w-full sm:w-2/3 lg:w-full lg:min-w-[100%] ml-1">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-y-2 gap-x-12 text-sm mb-6 text-left">
                  <div><strong>QA FORM NAME:</strong> {selectedForm}</div>
                  <div><strong>ACCOUNT:</strong> {selectedFormAccount}</div>

                  <div><strong>AGENT NAME:</strong> {employeeName} [{selectedEmployeeId || '—'}]</div>
                  <div><strong>LOB:</strong> {selectedFormData.LOB || '—'}</div>

                  <div><strong>SUPERVISOR NAME:</strong> {
                    isManualEntry
                      ? manualSupervisorName || '—'
                      : allEmployees.find(emp => emp.employeeId === selectedEmployeeId)?.supervisorName || '—'
                  } [
                  ...
                  <div><strong>TASK:</strong> {selectedFormData.TASK || '—'}</div>
                  <div><strong>EVALUATOR NAME:</strong> {userName}</div>
                  <div><strong>EVALUATION DATE:</strong> {new Date().toLocaleString()}</div>
                  <div><strong>EVALUATION TYPE:</strong> {formData.evaluationType || '—'}</div>
                  <div><strong>QA SCORE: </strong>     
                    <span className={`font-bold text-xl ml-2 ${qaScore >= 95 ? "text-green-700" : qaScore >= 85 ? "text-yellow-600" : "text-red-600"}`}>
                      {qaScore.toFixed(2)}%
                    </span>
                  </div>
                </div>
                  ...
                  <table className="w-full table-fixed border border-gray-300 text-sm">
                    <thead>
                      <tr className="bg-blue-900 text-white">
                        <th className="text-left px-4 py-2 w-[25%]">Section</th>
                        <th className="text-left px-4 py-2 w-[45%]">Line Item</th>
                        <th className="text-center px-4 py-2 w-[30%]">Observation</th>
                      </tr>
                    </thead>
                    <tbody>
                      {Object.entries(groupedData).map(([compKey, compData]) => {
                        const { title, questions } = compData;
                        const qEntries = Object.entries(questions);
                        const isZTP = compKey === "C011";

                        return qEntries.map(([qKey, qVal], idx) => (
                          <tr key={`${compKey}-${qKey}`} className="border-t border-gray-200">
                            {idx === 0 && (
                              <td
                            ...
                            <td className="pl-2 pr-8 py-2 w-[40%] break-words whitespace-normal">{qVal.QUESTION || "—"}</td>
                            <td className="text-center px-4 py-2 w-[30%]">
                              {(isZTP ? ["Flagged", "N/A"] : ["Met", "Not Met", "N/A"]).map((opt) => (
                                <label key={opt} className="inline-flex items-center mr-6">
                                  <input
                                    ...
                                    onChange={() => handleObservationChange(qKey, opt, groupedData)}
                                    className="mr-1"
                                  />
                                  {opt}
                                </label>
                    ...
                    <tfoot>
                      <tr className="border-t border-gray-300">
                        <td colSpan={3} className="px-4 py-4 text-left align-top">
                          <div>
                            <label htmlFor="observation-comments" className="block text-md font-bold text-black mb-1">
                            ...
                            <textarea
                              ...
                              onChange={(e) =>
                                setFormData((prev) => ({ ...prev, observationComments: e.target.value }))
                              }
        ...
        <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center transition-opacity duration-200">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6 animate-modalEnter">
            <div className="flex items-center mb-4">
              ...
              <h2 className={`text-xl font-bold ${saveSuccess ? "text-green-600" : "text-red-600"}`}>
                {saveSuccess ? "QA Audit saved successfully!" : "Failed to save QA Audit."}
              </h2>
            </div>
